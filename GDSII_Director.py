...
class GDSII_Director(object):
    ...
    def drawSquareInverse(self, pitch, width):
        ...
        xy1 = np.array([0,0,0,pitch,tmp[0],pitch,tmp[0],0,0,0],dtype=np.int32)
        xy2 = np.array([tmp[0],0,tmp[0],tmp[1],tmp[2],tmp[3],tmp[4],tmp[5],
                        tmp[6],tmp[7],tmp[0],pitch,pitch,pitch,pitch,0,tmp[0],
                        0],dtype=np.int32)
